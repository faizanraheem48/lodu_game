    ...
    if (msg.type === 'createRoom') {
      ...
      if (movable.length === 0) {
        ...
        setTimeout(() => {
          ...
          const extraTurn = dice === 6 || (result && result.captured);
          broadcast(state, { type: 'moved', color: currentColor, tokenIdx: movable[0], tokens: state.tokens, captured: result ? result.captured : false, attackerColor: currentColor });
          nextTurn(state, extraTurn);
        }, 250);
      } else {
        // Let player choose
      ...
      broadcast(state, { type: 'moved', color: currentColor, tokenIdx, tokens: state.tokens, captured: result ? result.captured : false, attackerColor: currentColor });
      nextTurn(state, extraTurn);
    }
  });
